...
const _helper = _interopRequireDefault(require('../helper'));

const _user = _interopRequireDefault(require('../models/user.model'));

const _helper2 = _interopRequireWildcard(require('./helper'));

const retrieveUser = function retrieveUser(req, res) {
  const user = _user.default;
...
const validateRule = function validateRule(req, res) {
  try {
    ...
    if (data.constructor === Array || typeof data === 'string') {
      ...
      const result = (0, _helper2.default)(data[rule.field], rule.condition, rule.condition_value);
    ...
    if ((0, _typeof2.default)(data) === 'object') {
      ...
      for (let _i = 0, _Object$keys = Object.keys(data); _i < _Object$keys.length; _i++) {
        const iterator = _Object$keys[_i];
        ...
        if (rule.field.includes('.')) {
          ...
          if (iterator === field) {
            // eslint-disable-next-line no-restricted-syntax
            for (let _i2 = 0, _Object$keys2 = Object.keys(data[iterator]); _i2 < _Object$keys2.length; _i2++) {
              const iterator2 = _Object$keys2[_i2];

              if (iterator2 === nestedField) {
                const _result = (0, _helper2.default)(data[iterator][iterator2], rule.condition, rule.condition_value);
          ...
          const _result2 = (0, _helper2.default)(data[iterator], rule.condition, rule.condition_value);
